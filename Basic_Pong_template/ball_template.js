...
export default class Ball extends THREE.Mesh {
    ...
    update(deltaT) {
        ...
        this.center=this.center.add(centerIncrement);
        ...
        if (centerIncrement.x<0.0) { // The ball is moving to the left
            if (this.center.x-this.radius<=this.player1.center.x+this.player1.halfSize.x &&
                ...
                this.center.y<=this.player1.center.y+this.player1.halfSize.y&&
                this.center.x+this.radius<=this.player2.center.x-this.player1.halfSize.x) { // The ball hit player 1 racket
        ...
        /* To-do #12 - Check if the ball hit player 2 racket
            ...
            if ( this.center.x+this.radius>=this.player2.center.x-this.player2.halfSize.x&&
                ...
                this.center.y<=this.player2.center.y+this.player2.halfSize.y &&
                this.center.x-this.radius<=this.player2.center.x+this.player2.halfSize.x) { // The ball hit player 2 racket
                this.direction = Math.PI-this.direction;  // The ball rebounds
            }
        } 
        ...
        this.center.y+this.radius>=this.table.halfSize.y) { // The ball is moving up and hit the top line
            ...
            centerIncrement = new THREE.Vector2(coveredDistance*Math.cos(this.direction),coveredDistance*Math.sin(this.direction));
            this.center=this.center.add(centerIncrement);
        }
